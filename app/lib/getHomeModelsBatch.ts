import type { ModelItem } from "@/app/components/ModelCard";
...
export type HomeModelsBatch = {
  models: ModelItem[];
  hasMore: boolean;
};

// 홈페이지 모델 목록 + 판매자 프로필 + 댓글 수를 서버에서 병렬로 모아 60초간 캐싱한다.
// app/page.tsx(서버 컴포넌트)와 app/api/home-models/route.ts(더보기용)가
// 동일한 fetch 호출을 하도록 이 함수 하나로 모아, Next Data Cache 키가 갈리지 않게 한다.
export async function getHomeModelsBatch(offset: number): Promise<HomeModelsBatch> {
  ...
  return { models: mapped, hasMore: models.length === MODELS_LIMIT };
}
