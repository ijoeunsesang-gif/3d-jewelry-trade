...
export default function FavoritesPage() {
  ...
  const removeFavorite = async (modelId: string) => {
    try {
      ...
      if (error) { showError("Ï∞??¥Ï†ú???§Ìå®?àÏäµ?àÎã§."); return; }
  ...
  return (
    <main style={{ maxWidth: 1200, margin: "40px auto", padding: "0 20px", fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ margin: 0, fontSize: 28, fontWeight: 900, color: "#111827" }}>Ï∞úÌïú Î™®Îç∏</h1>
        <p style={{ margin: "6px 0 0", color: "#6b7280", fontSize: 14 }}>
          ?Ä?•Ìï¥??Í¥Ä??Î™®Îç∏????Î≤àÏóê Î™®ÏïÑÎ≥¥ÏÑ∏??
        </p>
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          >
            ...
            <option value="oldest">?§Îûò?úÏàú</option>
      ...
      {/* Î™©Î°ù */}
      {loading ? (
        <p style={{ color: "#6b7280" }}>Ï∞?Î™©Î°ù Î∂àÎü¨?§Îäî Ï§?..</p>
      ) : models.length === 0 ? (
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 24, padding: 32, background: "white" }}>
          <p style={{ fontSize: 15, color: "#6b7280", margin: "0 0 16px" }}>?ÑÏßÅ Ï∞úÌïú Î™®Îç∏???ÜÏäµ?àÎã§.</p>
          <Link href="/" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", height: 48, padding: "0 18px", borderRadius: 14, background: "#111827", color: "white", textDecoration: "none", fontWeight: 800 }}>?ÅÌíà Î≥¥Îü¨Í∞ÄÍ∏?/Link>
        </div>
            ...
            return (
              <div key={item.id} style={{ border: "1px solid #e5e7eb", borderRadius: 20, background: "white", overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 2px 12px rgba(15,23,42,0.06)" }}>
                ...
                <div style={{ padding: "0 14px 14px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
                  ...
                  >
                    {removingId === item.id ? "?¥Ï†ú Ï§?.." : "Ï∞??¥Ï†ú"}
                  </button>
                </div>
              </div>
            );
