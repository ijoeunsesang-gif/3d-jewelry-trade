  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#6b7280", fontSize: 14, marginBottom: 40 }}>
        시행일: 2025년 1월 1일&nbsp;&nbsp;|&nbsp;&nbsp;최종 개정일: 2026년 5월 17일
      </p>
      ...
      <section style={sectionStyle}>
        <h2 style={h2Style}>제16조 (캐드스쿨 서비스)</h2>
        <p>① 회사는 이용자에게 CAD 관련 질문·답변, 수정 요청, 검수, 패키지 학습 지원 등 캐드스쿨 서비스를 제공합니다.</p>
        <p>② 캐드스쿨 서비스는 무료 서비스와 유료 서비스로 구분되며, 세부 이용 조건은 서비스 내 별도 안내에 따릅니다.</p>
        <p>③ 유료 서비스는 수강 패키지(기간제), 건별 질문, 이용권 추가 구매로 구성되며, 각 서비스의 내용 및 금액은 결제 전 화면에 명시됩니다.</p>
      </section>

      <section style={sectionStyle}>
        <h2 style={h2Style}>제17조 (답변 및 콘텐츠에 대한 책임 제한)</h2>
        <p>① 캐드스쿨 내 제공되는 답변, 수정 의견, 검수 의견 및 교육 콘텐츠는 일반적인 정보 제공 및 학습 지원을 목적으로 합니다.</p>
        <p>② 회사 및 멘토는 해당 내용의 완전성, 정확성, 특정 목적에 대한 적합성을 보증하지 않습니다.</p>
        <p>③ 이용자는 자신의 판단과 책임 하에 이를 활용해야 하며, 이를 이용하여 발생한 제작 오류, 손해 또는 분쟁에 대해 회사는 책임을 지지 않습니다.</p>
      </section>

      <section style={sectionStyle}>
        <h2 style={h2Style}>제18조 (수정 및 검수 서비스 범위)</h2>
        <p>① 유료 수정 및 검수 서비스는 상품별 안내된 범위 및 횟수 내에서 제공됩니다.</p>
        <p>② 멘토는 합리적인 범위를 초과하는 반복 수정 요청, 또는 최초 요청 내용과 현저히 다른 추가 작업에 대해 별도 비용을 청구하거나 요청을 거부할 수 있습니다.</p>
        <p>③ 검수 서비스는 디자인·출력·주조 가능성 등에 대한 의견 제공 서비스이며, 실제 제작 결과를 완전히 보증하지 않습니다.</p>
        <p>④ 수강 패키지의 잔여 횟수는 패키지 만료일 이후 소멸되며, 환불 또는 이월되지 않습니다.</p>
      </section>

      <section style={sectionStyle}>
        <h2 style={h2Style}>제19조 (캐드스쿨 유료 서비스 환불)</h2>
        <p>① 디지털 콘텐츠 및 온라인 서비스의 특성상, 아래 각 호에 해당하는 경우 환불이 제한될 수 있습니다.</p>
        <ul style={ulStyle}>
          <li>멘토로부터 답변 또는 수정 파일이 제공된 경우</li>
          <li>수강 패키지 이용 기간이 개시된 후 서비스를 1회 이상 이용한 경우</li>
          <li>건별 질문에서 횟수가 차감된 경우</li>
        </ul>
        <p>② 단, 회사의 귀책사유로 정상적인 서비스 제공이 불가능한 경우에는 관련 법령에 따라 환불 또는 보상이 이루어질 수 있습니다.</p>
        <p>③ 수강 패키지 미사용 상태에서 이용 기간 개시 전에는 전액 환불이 가능합니다.</p>
      </section>

      <section style={sectionStyle}>
        <h2 style={h2Style}>제20조 (게시물 및 질문 콘텐츠의 책임)</h2>
        <p>① 캐드스쿨 내 게시글, 질문, 첨부 파일 등 이용자가 작성·업로드한 콘텐츠에 대한 저작권 및 책임은 작성자 본인에게 있습니다.</p>
        <p>② 타인의 저작물을 무단으로 사용하거나 불법적인 파일을 업로드하는 경우, 해당 이용자가 모든 법적 책임을 집니다.</p>
        <p>③ 회사는 신고 접수 또는 자체 판단에 따라 해당 콘텐츠를 사전 통보 없이 삭제할 수 있습니다.</p>
      </section>
