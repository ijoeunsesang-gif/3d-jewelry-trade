...
const CATEGORY_LABEL: Record<string, string> = {
  ALL: "?„ì²´", RING: "ë§?, PENDANT: "?¬ë˜??, EARRING: "?´ì–´ë§?, BRACELET: "ë¸Œë ˆ?´ìŠ¬ë¦?, SET: "?¸íŠ¸",
};
...
export default function LibraryPage() {
  ...
  // ?˜ì´ì§€?¤ì´??  const [currentPage, setCurrentPage] = useState(1);
  ...
  const fetchLibrary = async () => {
    try {
      ...
      if (!session?.user) { showInfo("ë¡œê·¸?¸ì´ ?„ìš”?©ë‹ˆ??"); window.location.href = "/auth"; return; }
  ...
  const handleDownload = async (item: PurchasedModel) => {
    try {
      ...
      if (!res.ok) { showError(data.error || "?¤ìš´ë¡œë“œ ë§í¬ ?ì„±???¤íŒ¨?ˆìŠµ?ˆë‹¤."); return; }
      window.open(data.signedUrl, "_blank");
    } catch (e) { console.error(e); showError("?¤ìš´ë¡œë“œ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤."); }
  ...
  if (loading) {
    return <main style={{ maxWidth: 1100, margin: "40px auto", padding: "0 20px" }}><p>???¤ìš´ë¡œë“œë¥?ë¶ˆëŸ¬?¤ëŠ” ì¤?..</p></main>;
  }
      ...
      <main style={{ maxWidth: 1200, margin: "40px auto", padding: "0 20px", fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap", marginBottom: 16 }}>
            <div>
              <h1 style={{ fontSize: 30, fontWeight: 900, color: "#111827", margin: 0 }}>???¤ìš´ë¡œë“œ</h1>
              <p style={{ color: "#6b7280", fontSize: 14, margin: "6px 0 0" }}>êµ¬ë§¤??3D ëª¨ë¸??êµ¬ë§¤?¼ë¡œë¶€??6ê°œì›” ?™ì•ˆ ?ˆì „?˜ê²Œ ?¤ì‹œ ?¤ìš´ë¡œë“œ?????ˆìŠµ?ˆë‹¤.</p>
            </div>
            <div style={{ padding: "8px 14px", borderRadius: 999, background: "#f3f4f6", color: "#111827", fontWeight: 800, fontSize: 13 }}>ì´?{items.length}ê°?/div>
          </div>
        ...
        {items.length === 0 ? (
          <div style={{ border: "1px solid #e5e7eb", borderRadius: 24, padding: 32, background: "white" }}>
            <p style={{ fontSize: 16, color: "#6b7280", marginBottom: 16 }}>?„ì§ êµ¬ë§¤???í’ˆ???†ìŠµ?ˆë‹¤.</p>
            <Link href="/" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", height: 48, padding: "0 18px", borderRadius: 14, background: "#111827", color: "white", textDecoration: "none", fontWeight: 800 }}>?í’ˆ ë³´ëŸ¬ê°€ê¸?/Link>
          </div>
        ) : filteredItems.length === 0 ? (
          <div style={{ border: "1px solid #e5e7eb", borderRadius: 24, padding: 32, background: "white", textAlign: "center" }}>
            <p style={{ fontSize: 15, color: "#6b7280" }}>ê²€??ê²°ê³¼ê°€ ?†ìŠµ?ˆë‹¤.</p>
          </div>
        ) : (
              ...
              return (
                <div key={item.id} style={{ border: "1px solid #e5e7eb", borderRadius: 20, background: "white", overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 2px 12px rgba(15,23,42,0.06)" }}>
                  ...
                  <div style={{ padding: "14px 16px", flex: 1, display: "flex", flexDirection: "column", gap: 5 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#9ca3af" }}>êµ¬ë§¤??{purchaseDate}</div>
                    <div style={{ fontSize: 12, color: isExpired ? "#ef4444" : "#9ca3af" }}>
                      {isExpired ? `?¤ìš´ë¡œë“œ ê¸°í•œ ë§Œë£Œ (${expiresDateStr})` : `?¤ìš´ë¡œë“œ ê¸°í•œ ${expiresDateStr}ê¹Œì?`}
                    </div>
                  </div>
                  ...
                  <div style={{ padding: "0 14px 14px", display: "flex", flexDirection: "column", gap: 7 }}>
                    ...
                    >
                      {downloadingId === item.id ? "?ì„± ì¤?.." : isExpired ? "ê¸°í•œ ë§Œë£Œ" : "?¤ìš´ë¡œë“œ"}
                    </button>
